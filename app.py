...
def home():
   ...
   for idx, p in enumerate(properties):
       images = p.get("images", ["hero.png"])
       images_json = json.dumps(images)
       slides = ""
       dots = ""
       for i, img in enumerate(images):
           active = "active" if i == 0 else ""
           slides += f'<div class="slide {active}"><img src="/static/{img}" alt="תמונה {i+1}" data-images=\'{images_json}\' data-index="{i}"></div>'
...
<section class="hero">
   <div class="logo-wrap">
       <img src="/static/logo.png" alt="ARI נדלן">
   </div>
   <div class="line"></div>
   <h1>
       כל הדירות למכירה
...
function updateLightbox() {{
   document.getElementById('lb-img').src = '/static/' + lbImages[lbIndex];
